...
public class Validacao {
	public static float validaSalario() {
		...
		try {
			do {
				System.out.print("Digite o salario (se estiver desempregado digite 0): ");
				salario = Leitura.getFloat();
				if (salario < 0)
					System.out.println("Salario invalido, digite um valor não negativo.");
			} while (salario < 0);
		} catch (InputMismatchException excecao) {
			System.out.println("Houve um erro. O tipo de dado deve ser um numero");
		}
		return salario;
	}
	...
	public static int validaIdade() {
		...
		try {
			...
			System.out.println("Houve um erro. O tipo de dado deve ser um numero inteiro.");
		}
		return idade;
	}
